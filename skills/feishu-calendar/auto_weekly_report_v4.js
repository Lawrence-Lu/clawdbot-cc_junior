...
function parseMeetingContent(content) {
    ...
    for (const line of lines) {
        ...
        // 检测表名作为子任务分隔
        if (trimmed.match(/^[a-zA-Z_]+_result/) || trimmed.match(/^[a-zA-Z_]+_table/) || trimmed.match(/^【.+】$/)) {
...
async function createWeeklyReportDoc(title, contentLines) {
    ...
    for (const line of contentLines) {
        ...
        // 一级标题
        if (line.startsWith('# ') || line.startsWith('【一级】')) {
            blocks.push({
                block_type: 1,  // Page
                page: { elements: [{ text_run: { content: line.replace(/^#\s+|^【一级】/, '') } }] }
            });
        }
        // 二级标题（加粗文本模拟）
        else if (line.startsWith('## ') || line.startsWith('【二级】')) {
            blocks.push({
                block_type: 2,  // Text
                text: { 
                    elements: [{ 
                        text_run: { 
                            content: line.replace(/^##\s+|^【二级】/, ''),
        ...
        // 三级标题
        else if (line.startsWith('### ') || line.startsWith('【三级】')) {
            blocks.push({
                block_type: 2,
                text: { 
                    elements: [{ 
                        text_run: { 
                            content: line.replace(/^###\s+|^【三级】/, ''),
        ...
        // 列表项
            ...
            blocks.push({
                block_type: 7,  // Bullet
                bullet: { elements: [{ text_run: { content: line.replace(/^[-•]\s+/, '') } }] }
            });
        }
        // 普通文本
        else {
...
async function generateWeeklyReport() {
    ...
    // 标题
    contentLines.push(`【一级】本周工作周报（${today}）`);
    contentLines.push('');
    
    // 第一部分：进度
    contentLines.push('【一级】一、本周工作进度');
    contentLines.push('');
    
    let taskIndex = 1;
    for (const [category, items] of Object.entries(categories)) {
        // 分类作为二级标题
        contentLines.push(`【二级】${taskIndex}. ${category}`);
        contentLines.push('');
        
        for (const item of items) {
            // 任务标题作为三级标题
            contentLines.push(`【三级】${item.title}（${item.date}）`);
            if (item.meetingDoc) {
            ...
            for (const section of item.sections) {
                if (section.title) {
                    contentLines.push(`【子任务】${section.title}`);
                }
    ...
    // 第三部分：下一步计划
    contentLines.push('【一级】三、下周工作计划');
    contentLines.push('');
    
    let planIndex = 1;
    for (const [category, items] of Object.entries(categories)) {
        contentLines.push(`【二级】${planIndex}. ${category}`);
        contentLines.push('');
    ...
    try {
        ...
        console.log(`   - 层级结构: 一级标题(章节) > 二级标题(分类) > 三级标题(任务) > 列表项(具体内容)`);
